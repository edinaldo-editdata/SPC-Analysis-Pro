...
export const HistogramChart: React.FC<HistogramChartProps> = ({ histogramData, basicStats, lsl, usl }) => {
    ...
    const maxBinCount = Math.max(...histogramData.bins.map(b => b.y), 1);
    const maxNormalY = Math.max(...normalCurveData.map(p => p.y), 1);
    const scaledNormalCurve = normalCurveData.map(p => ({ x: p.x, y: (p.y / maxNormalY) * maxBinCount }));
                    ...
                    <ComposedChart data={chartData} margin={{ top: 20, right: 30, left: 0, bottom: 25 }}>
                        ...
                        {lsl !== null && (
                             <ReferenceLine yAxisId="left" x={lsl} stroke="#F56565" strokeWidth={2}>
                                <Label value={`LIE: ${lsl}`} position="top" fill="#F56565" />
                             </ReferenceLine>
                        )}
                        {usl !== null && (
                             <ReferenceLine yAxisId="left" x={usl} stroke="#F56565" strokeWidth={2}>
                                <Label value={`LSE: ${usl}`} position="top" fill="#F56565" />
                            </ReferenceLine>
                        )}

                        <ReferenceLine yAxisId="left" x={mean} stroke="#48BB78" strokeWidth={2}>
                            <Label value={`Lm: ${mean.toFixed(3)}`} position="top" fill="#48BB78" />
                        </ReferenceLine>
